...
{	
	$('[data-form-type="blocs-form"] input,[data-form-type="blocs-form"] textarea,[data-form-type="blocs-form"] select').jqBootstrapValidation(
    {
				...
				$.ajax({
		    		...
		    		success: function(data) // Success
		 			{  
		 				...
		 				// Handle JSON responses (Vercel API) or string responses (PHP)
		 				var responseData = typeof data === 'string' ? data : (data && data.success !== false ? 'success' : data);
		 				
		 				if (responseData == 'capture-error' || responseData == 'capture-connection-error') // Capture Error
		 				{
		 					var type = 'warning';

		 					if (responseData == 'capture-connection-error')
		 					{
		 						type = 'fail';
		 					}
			   		...
			   		error: function(xhr, status, error) // Error Posting Form
			   		{
						...
						try {
							...
							console.log("Mail Error: " + (response.message || error));
						} catch(e) {
	 ...
	 // Get Path to processor file (Vercel API or PHP fallback)
	 function getProcessorPath(form)
	 {
		var formId = form.attr('id');
		
		// Use Vercel API endpoint for contact form
		if (formId === 'contact_form') {
			return '/api/contact';
		}
		
		// Fallback to PHP for other forms (if needed)
		var path = siteRelativeURLPath+"includes/"+formId+".php";
		
		if (form.attr('template-path')) // Check For Template Path (Wordpress)
		{
			path = form.attr('template-path')+"/includes/"+formId+".php";
		}
